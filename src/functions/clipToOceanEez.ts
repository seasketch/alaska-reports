...
import alaskaClippingLayer from "./alaskaClippingLayer.json" with { type: "json" };
...
export async function clipToOceanEez(
  ...
  const sketchUnclean = makeUnclean(feature, 160, 360) as Feature;

  const keepInsideEez: FeatureClipOperation = {
    operation: "intersection",
    clipFeatures: toFeatureArray(
      alaskaClippingLayer as FeatureCollection,
